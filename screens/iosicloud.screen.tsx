...
const IOSICloudScreen: FC<Props> = ({}) => {
  ...
  const [filePathForWrite, setFilePathForWrite] = useState(
    PathUtils.join(defaultICloudContainerPath, 'Documents/test-create/file-on-cloud.txt'),
  );
  ...
  const [localFilePathForWrite, setLocalFilePathForWrite] = useState(
    FileSystem.documentDirectory + '/local-file.txt',
  );
    ...
    <KeyboardAwareScrollView
      ...
      <Block>
        ...
        <Button
          ...
          onPress={() => {
            FileSystem.deleteAsync(localFilePathForDelete)
              .then(() => {
                console.log('deleted');
              })
              .catch(err => {
